/* ============================================
   Neon Prediction v11.3 — مع إصلاح الأونلاين
   ============================================ */
...
/* ============================================
   ONLINE GAME — النسخة المُصلَّحة v11.3
   ============================================ */
var Online = {
...
function subscribeToRoom(roomId) {
  ...
  // ⭐ Polling قوي للجولات (لكل اللاعبين)
  ...
  Online.roundPollInterval = setInterval(async function() {
    ...
    try {
      ...
      if (r.data && r.data.round_number > 0 && r.data.round_number !== Online.currentRound) {
        console.log('🎯 Polling: New round detected:', r.data.round_number, 'current:', Online.currentRound);
        startOnlineRound(r.data);
      }
    } catch(e) { console.error('roundPoll:', e); }
  }, 1500);

  // ⭐ Polling لإنهاء الجولة (لو انتهى الوقت ومفيش Host رد)
  ...
  Online.roundEndPollInterval = setInterval(async function() {
    ...
    if (elapsed > Online.choiceTime + 3 && !Online.roundEnded) {
      console.log('⏰ Force ending round (timeout)');
...
function startOnlineRound(round) {
  // حماية من التكرار
  if (Online.roundData && Online.roundData.id === round.id) {
    console.log('⚠️ Same round ignored');
    return;
  }
  if (Online.currentRound >= round.round_number && Online.roundData && Online.roundData.id) {
    console.log('⚠️ Older round ignored');
    return;
  }
...
async function finishOnlineRound(round) {
  ...
  // ⭐ الهوست بس اللي يبدأ الجولة الجديدة
  if (Online.isHost) {
    setTimeout(function() {
      if (Online.playing) nextOnlineRound();
    }, 2000);
  }
}
...
console.log('✅ Neon Prediction v11.3 loaded — Online Fixed + Lucky Wheel v2 💎');
